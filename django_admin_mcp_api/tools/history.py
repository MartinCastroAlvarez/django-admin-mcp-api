"""`admin.history` — LogEntry timeline for one object."""
...
def _build_target(arguments: dict[str, Any]) -> DispatchTarget:
    return DispatchTarget(
        method="GET",
        path=f"/{arguments['app_label']}/{arguments['model_name']}/{arguments['pk']}/history/",
    )
...
TOOL = Tool(
    name="admin.history",
    description=(
        "Return the LogEntry timeline (admin history) for one object. Mirrors "
